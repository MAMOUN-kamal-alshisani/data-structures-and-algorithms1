...
function transformToLis(obj){
  // Solution code here...
  let TheArr=Object.entries(obj).map(val=>{
...
const totalSum = (input) => {
  // Solution code here...
  let Thearr= input.reduce((a,b) => a + b.reduce((a,b)=> a + b ,0),0); // error
  return Thearr;
};
...
let findMaleAndFemale = (data) => {
  ...
  let TheArr= data.filter(val=>val.gender==='female' || val.gender==='male').map(val=>val.name).join(' and ');
  return TheArr;
